...
import {type Watcher, type Config, makeWatcher} from './watcher';
...
class ServerlessOfflineWatcherPlugin implements Plugin {
	...
	constructor(private readonly serverless: Serverless) {
		...
		this.config.forEach((v, i) => {
			if (typeof v !== 'object') {
				throw new Error(`${PLUGIN_NAME}: config entries must be objects, but the entry at index ${i} was a ${typeof v}`);
			}
